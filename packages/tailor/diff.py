...
class RecruiterChange(BaseModel):
    """What a person is likely to make of the résumé, before and after.

    Beside `AtsChange` rather than folded into it, because they answer
    different questions and can move in opposite directions — which is the
    whole reason the recruiter axis exists. A rewrite that stuffs the posting's
    vocabulary into every bullet raises ATS keyword coverage and lowers this,
    and a single number would hide the trade the owner most needs to see.

    Four levels, carried apart for the same reason. `credibility` is the one
    that ends candidacies and the one tailoring cannot fix: a Skills list
    naming technologies the experience never shows reads as inflation to a
    person, however well it scores on a keyword match.
    """

    before: float = 0.0
    after: float = 0.0
    #: The recommendation, in §17's own words: strong yes / yes / maybe / no.
    shortlist_before: str = ""
    shortlist_after: str = ""
    scan_after: float = 0.0
    qualification_after: float = 0.0
    credibility_after: float = 0.0
    technical_after: float = 0.0
    #: What a reader would object to, most costly first. Human-readable
    #: already — these are rendered verbatim, so they are written as sentences.
    findings: list[str] = Field(default_factory=list)

    @property
    def regressed(self) -> bool:
        """The rewrite reads worse to a person than the source did.

        Always worth surfacing. The ATS score can be climbing while this falls,
        and that combination is exactly what an optimizer tuned on keywords
        alone produces.
        """
...
class DiffSummary(BaseModel):
    ...
    #: Same condition, same reason — both levels that need a posting would be
    #: inventing a reference without one.
    recruiter: RecruiterChange | None = None
...
def summarize(
    ...
    *,
    ...
    recruiter: RecruiterChange | None = None
    if source is not None and tailored is not None and job_description.strip():
        from packages.tailor import recruiter as recruiter_scorer

        first = recruiter_scorer.score(source, job_description)
        second = recruiter_scorer.score(tailored, job_description)
        recruiter = RecruiterChange(
            before=first.overall,
            after=second.overall,
            shortlist_before=first.shortlist,
            shortlist_after=second.shortlist,
            scan_after=second.scan,
            qualification_after=second.qualification,
            credibility_after=second.credibility,
            technical_after=second.technical,
            # The findings of the document being sent, not of the source. The
            # owner is deciding whether to approve *this* one.
            findings=[str(f) for f in sorted(second.findings, key=lambda f: -f.cost)],
        )
